...
try {
    ...
    Write-Host "   💡 Start it with: cd backend\api; C:\php81\php.exe artisan serve" -ForegroundColor Yellow
    $allGood = $false
}
...
try {
    ...
    Write-Host "   💡 Start it with: cd backend\ml-service; python app.py" -ForegroundColor Yellow
    $allGood = $false
}
...
try {
    ...
    Write-Host "   💡 Start it with: cd frontend\portal; npm run dev" -ForegroundColor Yellow
    $allGood = $false
}
...
try {
    ...
    Write-Host "   💡 Make sure Laravel API is running and database is set up" -ForegroundColor Yellow
    $allGood = $false
}
...
try {
    ...
    if ($response.success) {
        Write-Host "   ✅ Learning Summary API works" -ForegroundColor Green
        Write-Host "   📊 Documents processed: $($response.summary.total_documents)" -ForegroundColor Cyan
    }
} catch {
    Write-Host "   ⚠️  Learning Summary API not available (ML Service may need restart)" -ForegroundColor Yellow
}
...
if ($allGood) {
    ...
    Write-Host "`n💡 Start missing services and run this script again" -ForegroundColor Yellow
}
Write-Host "========================================`n" -ForegroundColor Cyan
